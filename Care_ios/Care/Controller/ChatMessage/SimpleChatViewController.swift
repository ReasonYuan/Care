...
var simpleChatViewControllers = NSMutableArray()
var SimpleChatViewControllerInstance:SimpleChatViewController?
class SimpleChatViewController: BaseViewController,UITableViewDataSource,UITableViewDelegate, UITextFieldDelegate,ComFqHalcyonLogicPracticeSendPatientLogic_SendPatientInterface,ComFqHalcyonLogicPracticeSendPatientLogic_SendRecordInterface,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate,ComFqHalcyonLogicPracticeUpLoadChatImageManager_UpLoadChatImageManagerCallBack{
    
    @IBOutlet var sendOtherView: UIView!
    ...
    var mIsShowOtherView:Bool = false
    
    var tmpEntity:ComFqHalcyonEntityChartEntity?
    var fromSendRecord:Bool = false
    ...
    override func viewDidLoad() {
        ...
        var customUser = "\(toUser!.getUserId())"
...
//        setRightBtnTittle("管理")
    ...
    override func viewWillAppear(animated: Bool) {
        ...
        if shareChatEntityList.size() != 0 {
            for i in 0..<shareChatEntityList.size() {
                ...
                MessageTools.saveSimplechatList("\(toUser!.getUserId())",text: shareChatEntity!.description(),success:true)
                
                MessageTools.sendMessage(shareChatEntity.description(), payLoad: "", type: 1, customId: "\(toUser!.getUserId())", callBackTag: "", toUser: toUser)
            }
    ...
    override func getXibName() -> String {
        return "SimpleChatViewController"
    }
    ...
    func submit() {
        ...
        if count(str) > 0 {
            ...
            MessageTools.saveSimplechatList("\(toUser!.getUserId())",text: messageEntity.description(),success:true)
            
            MessageTools.sendMessage(messageEntity.description(), payLoad: "", type: 1, customId: "\(toUser!.getUserId())", callBackTag: "", toUser: toUser)
            tabView.reloadData()
            if (tabView.contentSize.height - tabView.bounds.size.height) > 0 {
                tabView.setContentOffset(CGPointMake(0,tabView.contentSize.height - tabView.bounds.size.height), animated: true)
            }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if userId != ComFqLibToolsConstants.getUser().getUserId() {
            if messageType == 2 {
                ...
                cell?.userHead.downLoadImageWidthImageId(toUserImageId, callback: { (view, path) -> Void in
                    var tmpView = view as! UIImageView
                    UITools.getThumbnailImageFromFile(path, width: tmpView.frame.size.width, callback: { (image) -> Void in
                        tmpView.image = image
                    })
                ...
                cell?.head.downLoadImageWidthImageId(toUserImageId, callback: { (view, path) -> Void in
                    var tmpView = view as! UIImageView
                    UITools.getThumbnailImageFromFile(path, width: tmpView.frame.size.width, callback: { (image) -> Void in
                        tmpView.image = image
                    })
                ...
                var tmpView = UIView(frame: CGRectMake(65, 38,CGFloat(entity!.getImageWidth()), CGFloat(entity!.getImageHeight())))
    ...
    @IBAction func sendOtherClick(sender: AnyObject) {
    ...
    func imagePickerController(picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [NSObject : AnyObject]) {
        ...
        MessageTools.saveSimplechatList("\(toUser!.getUserId())",text: messageImageEntity.description(),success:true)
        MessageTools.setSendRecentContact("\(toUser!.getUserId())", text: messageImageEntity.description(), chatType: 1, imageId: Int(toUser!.getImageId()), name: toUser!.getName())
        
        var customUser = "\(toUser!.getUserId())"
        
        ComFqHalcyonLogicPracticeUpLoadChatImageManager.getInstanceWithNSString(customUser).upLoadImageWithNSString(path + name, withInt: messageImageEntity.getMessageIndex(), withComFqHalcyonLogicPracticeUpLoadChatImageManager_UpLoadChatImageManagerCallBack: self, withNSString: customUser,withInt:2,withInt:toUser!.getImageId(),withNSString:toUser!.getName())
    ...
    func sendRecordNotification(notification:NSNotification) {
        var userInfo = notification.userInfo!
        var messageEntity: ComFqHalcyonEntityChartEntity = userInfo["sendmessage"] as! ComFqHalcyonEntityChartEntity
        MessageTools.sendMessage(messageEntity.description(), payLoad: "", type: 1, customId: "\(toUser!.getUserId())", callBackTag: "", toUser: toUser)
        showOrHidenView(false)
    }
    ...
    override func onRightBtnOnClick(sender: UIButton) {
        ...
        var controller = ManageViewController()
    ...
    func sendPatientList(){
        ...
        for i in 0..<patientList.size() {
            ...
            sendPatientLogic.sendPatientToUserWithComFqHalcyonLogicPracticeSendPatientLogic_SendPatientInterface(self, withInt: toUser!.getUserId(), withInt: patientabstract.getPatientId(),withComFqHalcyonEntityPracticePatientAbstract:patientabstract,withInt:isSend)
        }
        patientList.clear()
    }
    ...
    func sendReocordList(){
        ...
        for i in 0..<recordList.size() {
            ...
            sendPatientLogic.sendRecordToUserWithComFqHalcyonLogicPracticeSendPatientLogic_SendRecordInterface(self, withInt: toUser!.getUserId(), withInt: recordabstract.getRecordItemId(),withComFqHalcyonEntityPracticeRecordAbstract:recordabstract,withInt:isSend)
        }
        recordList.clear()
    }
    ...
    func onSendPatientSuccessWithInt(shareMessageId: Int32, withInt sharePatientId: Int32,withComFqHalcyonEntityPracticePatientAbstract obj: ComFqHalcyonEntityPracticePatientAbstract!) {
        ...
        MessageTools.saveSimplechatList("\(toUser!.getUserId())",text: chatEntity.description(),success:true)
        
        MessageTools.sendMessage(chatEntity.description(), payLoad: "", type: 1, customId: "\(toUser!.getUserId())", callBackTag: "", toUser: toUser)
        if position == Int(patientList.size()){
            position = 0
            loadingDialog.close()
            if errorSendCount != 0 {
