...
public class HTTPTask : NSObject, NSURLSessionDelegate, NSURLSessionTaskDelegate {
            ...
            completionHandler: {(data: NSData!, response: NSURLResponse!, error: NSError!) -> Void in
                if let handler = completionHandler {
                    ...
                    if let d = data {
                        ...
                        if(self.responseSerializer != nil && response != nil){
                            let resSerializer = self.responseSerializer
                            let resp = response
                        ...
                        if(serialResponse.statusCode != nil && serialResponse.statusCode > 299){
                            let code = serialResponse.statusCode
    ...
    */
    private func createRequest(url: String, method: HTTPMethod, parameters: Dictionary<String,AnyObject>!) -> (request: NSURLRequest, error: NSError?) {
        ...
        if(self.baseURL != nil && !url.hasPrefix("http")){
    ...
    */
    ...
    public func URLSession(session: NSURLSession, task: NSURLSessionTask, didReceiveChallenge challenge: NSURLAuthenticationChallenge, completionHandler: (NSURLSessionAuthChallengeDisposition, NSURLCredential!) -> Void) {
        if(security != nil && challenge.protectionSpace.authenticationMethod == NSURLAuthenticationMethodServerTrust){
            let sec = security
        ...
        if session.configuration.valueForKey("identifier") != nil { //temp workaround for radar: 21097168
            if let blocks = backgroundTaskMap[session.configuration.identifier] {
                if let handler = blocks.completionHandler {
                    ...
                    if(resp.statusCode != nil && resp.statusCode > 299){
