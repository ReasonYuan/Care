...
//        CGPoint viewPoint = [point convertPoint:locationPoint fromView:self];
        if(CGRectContainsPoint(CGRectMake(-k_POINT_WIDTH/2.f + point.frame.origin.x, -k_POINT_WIDTH/2.f + point.frame.origin.y, k_POINT_WIDTH*2, k_POINT_WIDTH*2), locationPoint))
...
//        [self setNeedsDisplay];
    ...
    else
    {
        ...
        BOOL check = cv::CameraScanner::isSquire([self getPoints]);
        if(check){
            [self setLineColor:[UIColor yellowColor]];
        }else {
            [self setLineColor:[UIColor redColor]];
        }
        if(self.delegate && check != _isSquare && [self.delegate respondsToSelector:@selector(onStatusChange:)]){
            [self.delegate onStatusChange:check];
        }
        _isSquare = check;
        [self setNeedsDisplay];
    }
